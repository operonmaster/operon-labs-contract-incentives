...
import {
  buildPaymentPolicyControlEvidence,
  buildPaymentPolicyEvidence,
  toPaymentPolicyFailureCode
} from "./payment-policy-evidence";
...
// Demo SLA uses a simple UTC business-day window; production calendars belong in plan policy/config.
...
async function settleAppealPacket(
  ...
  if (policies.length > 1) {
    ...
    const businessPolicyId = result.policyId;
    ...
    return {
      ...
      reason: summarizeAppealsReason(result.reasonCodes),
      reasonCodes: result.reasonCodes,
      policyId: businessPolicyId,
      policyControls: [...APPEALS_POLICY_CONTROLS],
      policyCriteria: buildAppealsPolicyCriteria(evidence, result.reasonCodes),
      audit,
      walletId: null
    };
  }
  ...
  try {
    ...
    const evidenceRecord = buildPaymentPolicyEvidence({
