...
Operon Labs Contract Incentives is a policy-gated incentive layer for healthcare operations. Business applications submit structured evidence about work performed. A shared AI/policy agent selects the applicable contract policy, evaluates the submitted evidence, computes the incentive amount, maps the approved submitter to a wallet, and executes a policy-defined token payment only when the policy conditions are met. The submitted Hedera bounty demo executes HBAR testnet payments, while the platform policy model can represent future USDC, OPER, OPRN, or another explicitly configured token after token-transfer support is added.
...
- Enable policy-defined HBAR payments in the submitted demo, with the model supporting future USDC, OPER, or OPRN policies after token-transfer support is added.
...
10. Execute HBAR payment through Hedera Agent Kit when the policy permits auto-settlement.
...
- Public demo.
- Feedback GitHub issue submitted to a Hedera Agent Kit JS or Python repository: https://github.com/hashgraph/hedera-agent-kit-js/issues/944
...
- demonstrate policy-constrained HBAR payment execution
...
- approved currency list: HBAR in the submitted demo; future USDC support requires a token-transfer executor
...
8. Hedera Agent Kit executes HBAR payment for approved auto-settlement cases.
...
- approved provider-documentation PAS events execute HBAR payment through Hedera Agent Kit without a manual plan-console approval button
...
- feedback issue link is ready for the bounty submission form: https://github.com/hashgraph/hedera-agent-kit-js/issues/944
...
- [x] Submit feedback issue to Hedera Agent Kit JS or Python repo: https://github.com/hashgraph/hedera-agent-kit-js/issues/944
- [x] Include feedback issue link in submission.
...
Operon Labs Contract Incentives is a policy-gated healthcare operations incentive layer: business apps submit evidence, policies determine whether contract incentives are earned, and Hedera executes auditable HBAR testnet payments only inside explicit plan-approved safety controls.
